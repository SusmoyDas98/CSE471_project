<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Find Your Perfect Roommate - DormLuxe</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link rel="stylesheet" href='/css/search_dorm_mate.css'>

</head>
<body>
    <x-page-header>
        <x-slot name="nav_links">
            <x-nav-buttons />
        </x-slot>
    </x-page-header>
    
    <div style="height: 100px;"></div>
    
    <div class="hero-section">
        <div class="container">
            <span class="hero-badge"><i class="fas fa-sparkles me-2"></i>AI Powered Matching</span>
            <h1 class="hero-title">Find Your <span>Perfect Roommate</span></h1>
            <p class="hero-subtitle">Connect with compatible roommates through our advanced AI matching system or search manually based on your preferences</p>
        </div>
    </div>
    
    <div class="container">
        <div class="search-container">
            <div class="search-header">
                <h3>Manual Search</h3>
                <p>Search for potential roommates by name or interests</p>
            </div>
            
            <div class="search-box mb-3">
                <input 
                    type="text" 
                    class="search-input" 
                    id="manualSearch"
                    placeholder="Search by user name"
                >
                {{-- <button class="search-btn" onclick="applyFilters()"> --}}
                <button class="search-btn" id="apply_search_btn" data-user-id="{{$user_id}}">
                
                    <i class="fas fa-search me-2"></i>Search
                </button>
            </div>
            
            <!-- Filter Section -->
            <div class="filter-box card p-3 mb-4 shadow-sm">
                <h5 class="mb-3">Filters</h5>
                <div class="row g-3">
                
                    <!-- Age Filter -->
                    <div class="col-md-4">
                        <label class="form-label">Age</label>
                        <div class="d-flex gap-2">
                            <input type="number" id="filterAgeMin" class="form-control" placeholder="Min" min="0">
                            <input type="number" id="filterAgeMax" class="form-control" placeholder="Max" min="0">
                        </div>
                    </div>
                
                    <!-- Gender Filter -->
                    <div class="col-md-4">
                        <label for="filterGender" class="form-label">Gender</label>
                        <select id="filterGender" class="form-select">
                            <option value="">Not Selected</option>

                            @forelse ($genders as $gender)
                            <option value="{{$gender}}">{{$gender}}</option>
                            @empty
                            <option value="">No options available</option>
                                
                            @endforelse
                        </select>
                    </div>
                
                    <!-- Dorm Name Filter -->
                    <div class="col-md-4">
                        <label for="filterDorm" class="form-label">Dorm Name</label>
                        <select id="filterDorm" class="form-select">
                        <option value="">Not Selected</option>
                            
                        @forelse ($all_dorms as $dorm)
                        <option value="{{$dorm->id}}">
                           {{$dorm->name}}, {{$dorm->location}} 
                        </option>
                        @empty
                            <option value="">No options available</option>                            
                        @endforelse
                        </select>
                    </div>
                    <!-- Marital Status Filter -->
                    <div class="col-md-4">
                        <label for="filterMaritalStatus" class="form-label">Marital Status</label>
                        <select id="filterMaritalStatus" class="form-select">
                            <option value="">Not Selected</option>
                        
                            @forelse ($marital_status as $status)
                                <option value="{{ $status }}">{{ $status }}</option>
                            @empty
                                <option value="">No options available</option>
                            @endforelse
                        </select>
                    </div>

                
                </div>
            
                <div class="mt-3 text-end">
                    {{-- <button class="btn btn-primary" onclick="applyFilters()"> --}}
                    <button class="btn btn-primary" id="apply_filter_btn" data-user-id="{{$user_id}}" >

                        Apply Filters

                    </button>
                </div>
            </div>


            
            <div class="divider">
                <span>Or try something smarter</span>
            </div>
            
            <div class="ai-mode-section">
                <i class="fas fa-brain ai-icon"></i>
                <h4>Switch to AI Mode</h4>
                <p>Let our AI analyze your profile and preferences to find the most compatible roommates for you</p>
                {{-- <button class="btn-ai-mode" onclick="activateAIMode({{$user_id}})"> --}}
                    <button class="btn-ai-mode" data-user-id = "{{$user_id}}">
                         <i class="fas fa-magic me-2"></i>Activate AI Match
                    </button>
                   
                {{-- </button> --}}
                <div class="usage-info">
                    <i class="fas fa-bolt"></i>
                    <span>3 AI searches remaining today</span>
                    <a href="#" style="color: var(--gold); margin-left: 10px; font-weight: 600;">Upgrade for unlimited</a>
                </div>
            </div>
        </div>
        
        <div class="results-container" id="resultsContainer">

            
                <!-- Results will be dynamically loaded here -->
                
            {{-- </div> --}}
        </div>
    </div>
    <div id="come_here"></div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script src = "/js/search_dorm_mate.js"></script>
    
</body>
</html>